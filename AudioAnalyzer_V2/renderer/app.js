...
// 应用版本号。初值只是"没跑在 Electron 里"时（例如浏览器直接打开 index.html）
// 的兜底显示；正常启动后会由 app:getVersion（即 package.json）覆盖。
// 这样升级版本只需改 package.json 一处，界面与导出报告自动跟随。
let APP_VERSION = 'v9.0.2';
...
function exportReport() {
  ...
  const A = STATE.analysis;
  const lines = [
    `=== Audio Analyzer Pro ${APP_VERSION} Report ===`,
    `File: ${F.filename}`,
    ...
    '',
    `Generated by Audio Analyzer Pro ${APP_VERSION}`,
...
window.addEventListener('DOMContentLoaded', () => {
  try {
    ...
    // 版本号：统一从 package.json 取（主进程 app.getVersion），
    // 省去在 HTML/报告模板里手写版本号 —— 那是升级时最容易漏改的地方。
    // 取不到时保留 HTML 里的初值，不阻塞启动。
    if (window.electronAPI && window.electronAPI.appVersion) {
      window.electronAPI.appVersion().then(v => {
        if (!v) return;
        APP_VERSION = 'v' + v;
        for (const id of ['appVersion', 'statVersion']) {
